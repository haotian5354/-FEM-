function [p,e,t]=preprocess(a,b,epsilon)
ellipse=[4,0,0,a,b,0,0,0,0,0]';
%列向量含义：4代表椭圆，a为长轴，b为短轴，本题中b取值可以变化
%rectangular=[3,4,0,epsilon*a/2,epsilon*a/2,0,0,0,epsilon*b/2,epsilon*b/2]';
...
%e的前两行为边缘单元中的边缘上的两个点的编号，第5行判断是什么边缘（共7条边缘），第6行判断为是否是夹杂单元,1为方板2为夹杂，第7行判断边缘是否为曲线（1为曲线）
figure(2)
pdemesh(p,e,t);
%绘制网格图
